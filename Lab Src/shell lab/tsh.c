...
void eval(char *cmdline) 
{
  //cmdline contains a string ends with \n
  char *argv[MAXARGS];
  char buf[MAXLINE];
  pid_t pid;
  int is_bg;
  ...
  if(!builtin_cmd(argv)){
    //needs to create a new process to run program in it.
    if((pid=fork())==0){
      //child process
      setpgid(0,0);
	...
	return;
      ...
      if(!is_bg){
	//it's a fg job
	//we have to wait to reap the child process
	sigset_t mask_all;
	sigfillset(&mask_all);
	sigprocmask(SIG_SETMASK,&mask_all,&mask);
...
void sigchld_handler(int sig) 
{
  ...
  while((pid=waitpid(-1,&status,WNOHANG|WUNTRACED))>0){
    //有可能是进程stop发出的,有可能是进程terminate发出的
    sigfillset(&mask);
    sigprocmask(SIG_SETMASK,&mask,&prev);
	...
	if(WIFEXITED(status)){
	  //被terminate并且是ok的
	  //操作jobs时需要mask signal
	  jid = pid2jid(pid);
      
	  int status_id = WEXITSTATUS(status);
	  sio_puts("sigchld_handler: Job [");
	  sio_putl(jid); sio_puts("] (");
	  sio_putl(pid); sio_puts("terminates OK (status ");
	  sio_putl(status_id);
	}
      }
...
void sigint_handler(int sig) 
{
  ...
  sigfillset(&mask);
  sigprocmask(SIG_SETBLOCK,&mask,&prev);
  ...
  if(pid){
    ...
    if(verbose){
      ...
      sio_putl(job->jid); sio_puts("] (");
      sio_putl(pid); sio_puts(") killed\n");
      sio_puts("sigtstp_handler: exiting\n");
    }
    kill(-pid,SIGINT);
  }

  sigprocmask(SIG_SETBLOCK,&prev,NULL);
  errno = old_errno;
}
